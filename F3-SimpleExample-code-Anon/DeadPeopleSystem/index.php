...
$f3->route('GET /',
  function ($f3) {
    ...
    echo Template::instance()->render('UserSelect.html');
  }
);
...
$f3->route('POST /search',
    function($f3) {
        ...
        $tofdeadth = $formdata['timeofdeath'];
        switch ($formdata['age'])
        {
            case "0～20":
                $agelow = 0;
        ...
        $list = $f3->get('DB')->exec("SELECT * FROM deadinfo WHERE (age BETWEEN'$agelow'and'$agehigh') AND (sex='$s') AND (height BETWEEN'$heightlow'and'$heighthigh') AND (height='$h') AND (tattoos='$t') AND (birthmark='$b') AND (timeofdeath='$tofdeadth')");
...
$f3->route('POST /AdmRegister',
    function($f3) {
        $formdata = array();			// array to pass on the entered data in
        $formdata["username"] = $f3->get('POST.Admuname');			// whatever was called "name" on the form
        $formdata["password"] = $f3->get('POST.AdmPassword');// whatever was called "colour" on the for
...
//		$f3->set('content','simplepet.html');							// will always go to simplepet after successful login
//		echo template::instance()->render('layout.html');
        header("location:/fatfree/DeadPeopleSystem/simpleform");                  // will always go to simplepet after successful login
    }
    else
...
$f3->route('POST /simpleform',
  function($f3) {
	...
	$formdata["name"] = $f3->get('POST.name');			// whatever was called "name" on the form
	$formdata["age"] = $f3->get('POST.age');// whatever was called "colour" on the for
    $formdata['dis_site']=$f3->get('POST.dis_site');
    $formdata['gender']=$f3->get('POST.gender');
    $formdata['description'] = $f3->get('POST.description');
...
$f3->route('GET /dataView',
  function($f3) {
    ...
    echo template::instance()->render('dataView.html');
  }
);
